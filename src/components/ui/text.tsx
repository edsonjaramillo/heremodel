...
export const textVariants = cva('antialiased', {
	variants: {
		...
		seperator: { true: 'border-b border-gray pb-2' },
	},
	compoundVariants: [
		{ size: ['xl', '2xl', '3xl', '4xl'], className: 'scroll-m-20 tracking-tight' },
	],
	defaultVariants: { size: 'base', textColor: 'black', seperator: false },
});

type Header1Props = React.ComponentProps<'h1'> & TextVariants;
export function H1({ children, className, seperator, size, textColor, ...props }: Header1Props) {
	const style = cn(
		textVariants({
			seperator,
			size: size || '4xl',
...
export function H2({ children, className, seperator, size, textColor, ...props }: Header2Props) {
	const style = cn(
		textVariants({
			seperator,
			size: size || '3xl',
...
export function H3({ children, className, seperator, size, textColor, ...props }: Header3Props) {
	const style = cn(
		textVariants({ seperator, size: size || '2xl', textColor, className: 'font-semibold' }),
		className
	);
	return (
...
export function H4({ children, className, seperator, size, textColor, ...props }: Header4Props) {
	const style = cn(
		textVariants({ seperator, size: size || 'xl', textColor, className: 'font-semibold' }),
		className
	);
	return (
...
export function Label({ children, className, seperator, size, textColor, ...props }: LabelProps) {
	const style = cn(
		textVariants({
			seperator,
			size: size || 'sm',
...
export function Span({ children, className, seperator, size, textColor, ...props }: SpanProps) {
